...
) (_ payment.PaymentStatus, err error) {
...
func (c *Client) WaitForPayment(
	...
	for time.Now().Before(deadline) {
		...
		} else if status == payment.PaymentStatusSuccess || status == payment.PaymentStatusFailed {
			return status, nil
		}
...
func (c *Client) GetTransactionInfo(
	ctx context.Context, transactionID string,
) (_ billingSvc.TransactionInfo, err error) {
...
// === Методы для реализации интерфейса billingSvc.TransactionInfo ===
